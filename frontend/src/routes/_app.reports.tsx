  ...
  head: () => ({
    meta: [
      { title: "Reports & Analytics — TransitOps" },
      {
        name: "description",
        content:
          "Fuel efficiency, utilization, operational cost, and per-vehicle ROI.",
      },
    ],
  }),
...
function ReportsPage() {
    ...
    .map((v, i) => {
      ...
      const km =
        trips
          ...
          .reduce((s, t) => s + t.distanceKm, 0) || 200 + i * 40;
      const revenue = km * 45 + i * 15000; // mock revenue rate
  ...
  const utilization = utilizationTrend.map((d) => ({
    ...d,
    cost: 8000 + d.trips * 900,
  }));
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Fuel efficiency, utilization, operational cost, and per-vehicle ROI."
        actions={
          <button className="inline-flex items-center gap-1.5 rounded-lg border border-border bg-card px-3 py-2 text-sm font-medium">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <ChartCard
          ...
          subtitle="Daily active fleet % against operational spend"
        >
        ...
        <ChartCard
          ...
          subtitle="Km per litre by vehicle (higher is better)"
        >
      ...
      <div className="overflow-hidden rounded-xl border border-border bg-card shadow-card">
        <div className="flex items-center justify-between border-b border-border px-5 py-4">
          ...
          <span className="inline-flex items-center gap-1 rounded-full bg-success/15 px-2.5 py-0.5 text-xs text-success">
            <TrendingUp className="size-3.5" /> Trailing 30d
          </span>
        </div>
        <table className="w-full text-sm">
          <thead className="bg-muted/40 text-xs uppercase tracking-wider text-muted-foreground">
            <tr>
              ...
              <th className="px-5 py-3 text-right">Maint. Cost</th>
              <th className="px-5 py-3 text-right">Revenue</th>
              <th className="px-5 py-3 text-right">Efficiency</th>
          ...
          <tbody className="divide-y divide-border">
            {rollup.map((r) => (
              <tr key={r.id} className="hover:bg-muted/30">
                ...
                <td className="px-5 py-3 text-right font-mono">
                  ₹{r.maint.toLocaleString("en-IN")}
                </td>
                <td className="px-5 py-3 text-right font-mono">
                  ₹{r.revenue.toLocaleString("en-IN")}
                </td>
                <td className="px-5 py-3 text-right font-mono">
                  {r.efficiency.toFixed(1)} km/L
                </td>
                <td
                  className={
