  ...
  head: () => ({
    meta: [
      { title: "Drivers — TransitOps" },
      {
        name: "description",
        content:
          "Driver roster with licence status, safety scores, and duty state.",
      },
    ],
  }),
...
function DriversPage() {
  return (
    <div className="space-y-6">
      <PageHeader
        title="Drivers"
        description="Compliance, safety scores, and duty status at a glance."
        actions={
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        {drivers.map((d) => {
          ...
          return (
            <div
              key={d.id}
              className="group rounded-xl border border-border bg-card p-5 shadow-card transition hover:border-primary/40"
            >
