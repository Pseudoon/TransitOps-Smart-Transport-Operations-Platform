          ...
          <tbody className="divide-y divide-border">
            {filtered.map((v) => (
              <tr key={v.id} className="hover:bg-muted/30">
                ...
                <td className="px-5 py-3 text-right font-mono">{v.capacityKg.toLocaleString()} kg</td>
                <td className="px-5 py-3 text-right font-mono">{v.odometer.toLocaleString()} km</td>
                <td className="px-5 py-3 text-right font-mono">₹{(v.acquisitionCost / 100000).toFixed(1)}L</td>
                <td className="px-5 py-3 text-muted-foreground">{v.region}</td>
                <td className="px-5 py-3"><StatusBadge status={v.status} /></td>
              </tr>
            ))}
            {filtered.length === 0 && (
              <tr><td colSpan={8} className="px-5 py-10 text-center text-muted-foreground">No vehicles match your filters.</td></tr>
            )}
          </tbody>
        </table>
      </div>
